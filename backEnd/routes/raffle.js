...
const resp = {checkID:"",isWin:false,desc:""};
...
router.post("/check", authenticate, async (req, res) => {
  ...
  try {
    ...
    const checkdCode= await collection.findOne({code:rCode});
    if(checkdCode){
   ...
   const checkID = await collection.insertOne(raffleCheck);

    //count checks

    const kountQuery = {
      customer: req.user.id,
      checkTime: { $gt: moment().startOf('day').format("YYYY-MM-DD hh:mm:ssss") },
    };
    ...
    const dbUser =  await dbClient.db(dbname).collection("customers").findOne({_id:ObjectId(req.user.id)});
    ...
    if(
      ...
      var update= await collection.updateOne(raffleCheck,{$set:rfl_update});
...
const dailyQuery = async function dailyQueryFn()
{
  ...
  try {
   ...
   const collection = dbClient.db(dbname).collection("rafflechecks");
    ...
    const query = {
      cat: {$exists:true, $in:['A','B','C','D']},
      name:{$exists:true},
      address:{$exists:true},
      checkTime:{$gte:Date(moment().add(-1, 'days').format('YYYY-MM-DD')),$lt:Date(moment().format('YYYY-MM-DD'))}
    };
    const results = await collection
                            .find(query)
                            .sort({checkTime: 1, cat:1})
                            .toArray();
...
//============Reporting===============================
module.exports = {router,dailyQuery};
